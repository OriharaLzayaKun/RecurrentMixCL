...
import os
import logging
import warnings     #Python的标准库模块，用于控制警告的产生和显示

warnings.filterwarnings("ignore")       #用来忽略警告
logging.getLogger("transformers.tokenization_utils_base").setLevel(logging.ERROR)   #设置日志记录器の日志级别


def tok_text(text):
    # 清洗文本，去除多余空格和换行符，并转换为小写
    return ' '.join(text.split()).strip().lower()       #token处理，去掉所有の换行符,统一为一句话
...
def load_wiki(file, head='Wikipedia denosing. '):
    #加载和处理Wikipedia数据文件，将一半的词随机替换为<mask>，返回上下文和响应
    def mask(text):
        text = text.split()
...
def load_batch_knowledge(file, title='Predict knowledge. '):
    # 加载批量知识数据文件，处理上下文和知识点，并返回处理后的数据。
    data = json.load(open(file))
    data_context = []
    data_response = []
    for example in data:
...
class Data(Dataset):
    # 一个PyTorch数据集类，用于加载和处理数据，将文本编码为token，并在获取数据时对其进行padding
...
class CLData(Dataset):
    # 继承自Data类，处理多语料数据，根据指定比例抽样数据，支持对比学习的数据加载
...
class BatchData(CLData):
    # 继承自CLData类，处理批量数据，支持设置负样本数量，获取数据时返回批量上下文、响应和标签
...
class SpanData(BatchData):
    # 继承自BatchData类，处理包含文本片段的数据，支持文本片段的替换和混合，用于生成训练数据
...
def main():
    ...
    wiki_context, wiki_response = load_wiki('dataset/wizard_wiki_full.json')
